...
from models.geometric_models.GraphNN import test as GraphNN_test
from parser import Parser
from test_metrics.TU_metrics.Agg import AgglomerativeClustering_metrics
from test_metrics.TU_metrics.GMM import GMM_metrics
from test_metrics.TU_metrics.Kmeans_selection import KMeans_score
from test_metrics.TU_metrics.M_random import random_select
from test_metrics.TU_metrics.Spectral import SpectralClustering_metrics
from test_metrics.TU_metrics.kmeans_plus import KMeans_plusplus_score
from test_metrics.TU_metrics.mini_batch import MiniBatchKMeans_metrics
...
def retrain_and_save(args):
    ...
    for exp in range(5):
        ...
        for exp_ID in range(args.mse_epochs):
            ...
            sample_test_acc = GraphNN_test(sample_loader, model)
...
def select_functions(model, retrain_dataset_length, metric, retrain_loader, select_num, dataset):
    if metric == "random":
        ...
        selected_index = MiniBatchKMeans_metrics(model, retrain_loader, select_num)
    elif metric == "kplus":
        selected_index = KMeans_plusplus_score(model, retrain_loader, select_num)
