...
def selectsample(args, model, x_test, delta, iterate, neuron_interval, neuron_proba, test_output, attack=0):
    test = x_test
    # print(test)
    batch = delta
...
def CES_selection(model, target_data, select_num):
    CES_index = conditional_sample(model, target_data, select_num)
    select_index = select_from_index(select_num, CES_index)
    return select_index
