...
def mcp_score(model, target_loader, num, class_num, batch_size):
    dicratio = [[] for i in range(class_num * class_num)]  # 只用90，闲置10个
    dicindex = [[] for i in range(class_num * class_num)]
    model.eval()
    with torch.no_grad():
        for batch_no, x_batch in enumerate(target_loader):
            x_batch = x_batch.to(device)
            output = model(x_batch)
            y_pred = torch.softmax(output, dim=1)
            for i in range(len(torch.Tensor.cpu(y_pred))):
                index_in_data = batch_size * batch_no + i
                act = torch.Tensor.cpu(y_pred)[i]
