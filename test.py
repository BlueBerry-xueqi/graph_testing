...
from test_metrics.ModelWrapper import ModelWrapper
...
pl=[]
def run():
        model_data_path="saved_model/gin_imdb_binary_/"
...
print("DSA {}, {}".format(np.mean(dl), np.var(dl)))
print("LSA {}, {}".format(np.mean(ll), np.var(ll)))
print("DeepGiNi {}, {}".format(np.mean(gl), np.var(gl)))
print("MCP {}, {}".format(np.mean(ml), np.var(ml)))
print("MaxP {}, {}".format(np.mean(pl), np.var(pl)))
