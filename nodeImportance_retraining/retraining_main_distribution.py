...
def get_retrain_distribution(rank_list, x, y, edge_index, ratio, epochs):
    ...
    for s in range(5):
        ...
        tmp_idx_1 = list(rank_list[:int(len(rank_list) * 0.2)])
        tmp_idx_2 = list(rank_list[int(len(rank_list) * 0.2):int(len(rank_list) * 0.4)])
        tmp_idx_3 = list(rank_list[int(len(rank_list) * 0.4):int(len(rank_list) * 0.6)])
        tmp_idx_4 = list(rank_list[int(len(rank_list) * 0.6):int(len(rank_list) * 0.8)])
        tmp_idx_5 = list(rank_list[int(len(rank_list) * 0.8):])

        select_n = int(int(len(rank_list)*ratio) / 5 + 1) + 1

        tmp_idx = list(random.sample(tmp_idx_1, select_n))+\
                  list(random.sample(tmp_idx_2, select_n))+\
                  list(random.sample(tmp_idx_3, select_n))+\
                  list(random.sample(tmp_idx_4, select_n))+\
                  list(random.sample(tmp_idx_5, select_n))
...
def main():
    ...
    for i in range(len(ratio_list)):
        ...
        degree_value = get_retrain_distribution(degree_rank_idx, x, y, edge_index, ratio, epochs)
        eccentricity_value = get_retrain_distribution(eccentricity_rank_idx, x, y, edge_index, ratio, epochs)
        center_value = get_retrain_distribution(center_rank_idx, x, y, edge_index, ratio, epochs)
        bc_value = get_retrain_distribution(bc_rank_idx, x, y, edge_index, ratio, epochs)
        ec_value = get_retrain_distribution(ec_rank_idx, x, y, edge_index, ratio, epochs)
        pr_value = get_retrain_distribution(pr_rank_idx, x, y, edge_index, ratio, epochs)
        hits_value = get_retrain_distribution(hits_rank_idx, x, y, edge_index, ratio, epochs)
