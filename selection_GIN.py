...
from trainer_GIN import GIN_train, GIN_test, construct_model
...
def loadData():
    ...
    if not (os.path.isfile(f"{savedpath_dataset}/train_index.pt") and os.path.isfile(
...
def retrain_and_save(args):
    ...
    for exp_ID in range(args.exp):
        ...
        for epoch in range(args.retrain_epochs):
            GIN_train(new_train_loader, model, optimizer)
            train_acc = GIN_test(new_train_loader, model)
