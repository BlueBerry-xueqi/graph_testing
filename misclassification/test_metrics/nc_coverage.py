...
class NCoverage():
    ...
    def initKMNCtable_split(self, input_data, file, read=False,save=True):
        '''
        Init k-multisection Neuron Coverage using the training data
        :param input_data:
        :return:
        ...
        for layer_name in self.layer_to_compute:
            ...
            for i in range(0, input_data.shape[0], 100):
                if i + 100 > input_data.shape[0]:
                    layer_outputs = layer_model.predict(input_data[i:])
                else:
                    layer_outputs = layer_model.predict(input_data[i:i + 100])

                reshape_layerouputs = layer_outputs.reshape(layer_outputs.shape[0], -1,
                                                            layer_outputs.shape[-1]) # num_sample, features, neurons
                reshape_layerouputs = np.mean(reshape_layerouputs, axis=1)# num_sample, neurons

                # neuron_max = np.squeeze([np.max(np.squeeze(np.max(reshape_layerouputs, axis=0))), neuron_max]) #neurons
                # neuron_min = np.squeeze([np.max(np.squeeze(np.min(reshape_layerouputs, axis=0))), neuron_min])#neurons
                # neuron_std = np.squeeze(np.var(reshape_layerouputs, axis=0))#neurons
                if i == 0:
                    neuron_max = np.squeeze(np.max(reshape_layerouputs, axis=0)) # neurons
                    neuron_min = np.squeeze(np.min(reshape_layerouputs, axis=0))  # neurons
                    ...
                    # print("##################")
                    # print(np.squeeze(np.max(reshape_layerouputs, axis=0)))
                    # print("##################")
                    # print(neuron_max)
                    neuron_max = np.max([np.squeeze(np.max(reshape_layerouputs, axis=0)), neuron_max], axis=0)
                    neuron_min = np.min([np.squeeze(np.min(reshape_layerouputs, axis=0)), neuron_min], axis=0) # neurons
                neuron_std = np.squeeze(np.var(reshape_layerouputs, axis=0))  # neurons
